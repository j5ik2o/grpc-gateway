...
object GatewayGenerator extends protocbridge.ProtocCodeGenerator {
  ...
  private def generateFile(fileDesc: FileDescriptor): CodeGeneratorResponse.File = {
    ...
    fileDesc.getName
    val objectName = fileDesc.getName.substring(0, fileDesc.getName.length - 5) + "Gateway"
    b.setName(s"${fileDesc.getFile.getName}/$objectName.scala")

    val fp = FunctionalPrinter()
      .add(s"package ${fileDesc.getName}")
      .newline
      .add(
