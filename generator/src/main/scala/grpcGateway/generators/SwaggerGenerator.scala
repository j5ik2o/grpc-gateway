...
object SwaggerGenerator extends protocbridge.ProtocCodeGenerator {
  ...
  private def generateFile(fileDesc: FileDescriptor): CodeGeneratorResponse.File = {
    ...
    val objectName = fileDesc.getName.substring(0, fileDesc.getName.length - 5)
    ...
    val fp = FunctionalPrinter()
      ...
      .add("paths:")
      .indent
      .print(paths) { case (p, m) => generatePath(m._1, m._2.toSeq)(p) }
      .outdent
      .add("definitions:")
      .indent
    ...
    field.getJavaType match {
      ...
      case JavaType.ENUM =>
        printer
          ...
          .addIndented(field.getEnumType.getValues.asScala.map(v => s"- ${v.getName}").toArray: _*)
      case JavaType.INT =>
        printer
          .add(s"- name: $prefix${field.getJsonName}")
    ...
    field.getJavaType match {
      ...
      case JavaType.ENUM =>
        _.add("type: string", "enum:").add(
          field.getEnumType.getValues.asScala.map(v => s"- ${v.getName}").toArray: _*
        )
